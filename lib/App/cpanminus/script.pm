...
sub resolve_name {
    ...
    # URL
    if ($self->get_scheme_backend($module)) {
...
sub _curl_backend {
    my($self, $curl) = @_;
    my @base_opts = ($curl, '-L', ( $self->{verbose} ? () : '-s' ));
    return {
        get => sub {
            my($self, $uri) = @_;
            $self->safeexec( my $fh, @base_opts, $uri ) or die "curl $uri: $!";
            local $/;
            <$fh>;
        },
        mirror => sub {
            my($self, $uri, $path) = @_;
            $self->safeexec( my $fh, @base_opts, '-#', '-o', $path ) or die "curl $uri: $!";
            local $/;
            <$fh>;
        },
    };
